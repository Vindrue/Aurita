...
pub fn render_status_bar(frame: &mut Frame, area: Rect, cas_status: &str, session_name: Option<&str>, flash: Option<&str>) {
    let version = env!("CARGO_PKG_VERSION");
    let left_text = match session_name {
        Some(name) => format!(" Aurita v{} | {}", version, name),
        None => format!(" Aurita v{}", version),
    };
    let right_text = format!("{} ", flash.unwrap_or(cas_status));
